...
from reid.insightface.utils import nms, adjust_input, detect_first_stage_warpper
    ...
    def detect_face_limited(self, img, det_type=2):
        ...
        for i in range(5):
            ...
            [dy, edy, dx, edx, y, ey, x, ex, tmpw,
             tmph] = self.pad(np.vstack([x, y, x + patchw - 1, y + patchw - 1]).T, width, height)
    ...
    def detect_face(self, img, det_type=0):
        ...
        if det_type == 0:
            ...
            total_boxes = np.vstack(
                [
                    total_boxes[:, 0] + total_boxes[:, 5] * bbw, total_boxes[:, 1] + total_boxes[:, 6] * bbh,
                    total_boxes[:, 2] + total_boxes[:, 7] * bbw, total_boxes[:, 3] + total_boxes[:, 8] * bbh,
                    total_boxes[:, 4]
                ]
            )
