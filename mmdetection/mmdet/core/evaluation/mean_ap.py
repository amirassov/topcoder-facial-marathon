...
def calc_confidence(det_results,
                    gt_bboxes,
                    gt_labels,
                    gt_ignore=None,
                    scale_ranges=None,
    ...
    for i in range(num_classes):
        ...
        tpfp_func = (tpfp_imagenet
                     if dataset in ['det', 'vid'] else tpfp_default)
        tpfp = [
            tpfp_func(cls_dets[j][cls_dets[j][:, -1] > score_threshold], cls_gts[j], cls_gt_ignore[j], iou_thr,
                      area_ranges) for j in range(len(cls_dets))
        ]
        tp, fp = tuple(zip(*tpfp))

        precisions = [x.sum() / len(y) for x, y in zip(tp, cls_dets)]
        recalls = [x.sum() / len(y) for x, y in zip(tp, cls_gts)]
